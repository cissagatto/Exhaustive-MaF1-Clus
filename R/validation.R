...
validateRUN <- function(number_dataset, number_cores, number_folds, folderResults){
  ...
  while(id_part<=n_bell){
    ...
    timeEP = system.time(resEPVAL <- validation(number_dataset, 
                                                number_cores,
                                                number_folds, 
                                                id_part, 
                                                folderResults))
    ...
    str5a = paste("cp ", diretorios$folderResults, 
                  "/", dataset_name, "-Partition-", id_part, 
                  "-validation-results.tar.gz ", 
                  diretorios$folderReVal, sep="")
    system(str5a)
    ...
    str4 = paste("rm -r ", diretorios$folderResults, "/", dataset_name, "-Partition-", id_part, "-validation-results.tar.gz ", sep="")
    system(str4)
    
    setwd(diretorios$folderResults)
    str5 = paste("rm -r ", dataset_name, "-Partition-", id_part,"-Evaluated.csv", sep="")
    system(str5)
