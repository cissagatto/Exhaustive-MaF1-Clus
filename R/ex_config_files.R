...
###############################################################################
FolderCF = paste(FolderRoot, "/EMaC-Config-Files", sep="")
...
while(i<=n){
  ...
  # Confi File Name
  file_name = paste(FolderCF, "/EMaC-", ds$Name, ".csv", sep="")
  ...
  write("Dataset_Path, ~/Exhaustive-MaF1-Clus/Datasets",
        file = output.file, append = TRUE)

  # job name
  job_name = paste("ex-", ds$Name, sep = "")
  ...
  # Absolute path to the bell partitions folder
  write("Partitions_Path, ~/Exhaustive-MaF1-Clus/Partitions/BellPartitions",
        file = output.file, append = TRUE)
