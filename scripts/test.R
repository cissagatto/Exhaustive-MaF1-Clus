##################################################################################################
# Exhaustive Partitions Macro F1                                                                 #
# Copyright (C) 2021                                                                             #
# JUNTA VALIDAÇÃO COM TREINO                                                                     #
#                                                                                                #
...
FolderRoot = ""
if (sistema[1] == "Linux"){
  FolderRoot = paste("/home/", sistema[7], "/Exhaustive-MacroF1-TVT", sep="")
} else {
  FolderRoot = paste("C:/Users/", sistema[7], "/Exhaustive-MacroF1-TVT", sep="")
}
setwd(FolderRoot)
...
bestPartitions <- function(dataset_name, number_folds, folderResults){
  ...
  write.csv(allMacroF1Part[-1,-2], paste(dataset_name, "-All-MacroF1-Exhaustive.csv", sep=""), row.names = FALSE)
  ...
  setwd(diretorios$folderResults)
  write.csv(result2[-1,], paste(dataset_name, "-Best-Partitions-Exhaustive.csv", sep=""), row.names = FALSE)
...
testEP <- function(ds, dataset_name, number_folds, namesLabels, folderResults){
  ...
  buildBellPartitions <- foreach(f = 1:number_folds) %dopar% {
    ...
    num.group = as.numeric(specificGroup$totalGroups)
    ...
    while(g<=num.group){
      ...
      classesVL = select(arquivoVL, grupo$labels)
...
asd <- function(ds, dataset_name, folderResults){
  ...
  # get the bell partitions for the dataset
  setwd(diretorios$folderBellPartDataset)
  nome2 = paste(dataset_name, "-groupsPerPartitions.csv", sep="")
  print(nome2)
  bell = data.frame(read.csv(nome2))
  bell = bell[,-1]
  
  # gather the group number with the respectively partition
  res1 = bell[bP$partition,]
  res2 = cbind(bP, res1)
  res3 = res2[,c(-1,-2)]
  res4 = res2[,-2]
  print(res4)
  ...
  media = apply(res3, 2, mean)
  mediana = apply(res3, 2, median)
  desvioPadrao = apply(res3, 2, sd)
